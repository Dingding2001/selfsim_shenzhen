...
with open(first_file, 'r', encoding='utf-8') as csvfile, open(second_file, 'w', encoding='utf-8',
                                                              newline='') as output_csvfile:
...
male_data['y'] = male_data['y'] / 1000
female_data['y'] = female_data['y'] / 1000
# 绘制折线图
...
# 设置图的标签、标题等
plt.xlabel('Year')
plt.ylabel('Number of People (Unit: Million)')
plt.xticks(male_data['x'], male_data['x'].tolist())
y_interval = 1  # 设置你希望的y轴间隔
...
plt.xlim(min(male_data['x'].min(), female_data['x'].min()) - 0.25,
         max(male_data['x'].max(), female_data['x'].max()) + 0.25)
plt.grid()
...
students_data['y'] = students_data['y']/1000
employee_data['y'] = employee_data['y']/1000
retiree_data['y'] = retiree_data['y']/1000
unemployee_data['y'] = unemployee_data['y']/1000

# 绘制折线图
plt.plot(students_data['x'], students_data['y'], label='Students', marker='o', color='green', linewidth=2,
         linestyle='dotted')
plt.plot(employee_data['x'], employee_data['y'], label='Employee', marker='o', color='blue', linewidth=2,
         linestyle='dotted')
plt.plot(retiree_data['x'], retiree_data['y'], label='Retiree', marker='o', color='orange', linewidth=2,
         linestyle='dotted')
plt.plot(unemployee_data['x'], unemployee_data['y'], label='Unemployee', marker='o', color='red', linewidth=2,
         linestyle='dotted')
...
y_interval = 2  # 设置你希望的y轴间隔

# 计算y轴的最小和最大值
y_min = min(students_data['y'].min(), employee_data['y'].min(), retiree_data['y'].min(),
            unemployee_data['y'].min()) - 0.5
y_max = max(students_data['y'].max(), employee_data['y'].max(), retiree_data['y'].max(),
            unemployee_data['y'].max()) + y_interval
...
plt.ticklabel_format(style='plain', axis='y')
plt.xlim(
    min(students_data['x'].min(), employee_data['x'].min(), retiree_data['x'].min(), unemployee_data['x'].min()) - 0.25,
...
highschool_data['y'] = highschool_data['y']/1000
college_data['y'] = college_data['y']/1000
bachelor_data['y'] = bachelor_data['y']/1000
master_phd_data['y'] = master_phd_data['y']/1000

# 绘制折线图
plt.plot(highschool_data['x'], highschool_data['y'], label='HighSchool or Below', marker='o', color='blue', linewidth=2,
         linestyle='dotted')
plt.plot(college_data['x'], college_data['y'], label='College', marker='o', color='red', linewidth=2,
         linestyle='dotted')
plt.plot(bachelor_data['x'], bachelor_data['y'], label='Bachelor', marker='o', color='green', linewidth=2,
         linestyle='dotted')
plt.plot(master_phd_data['x'], master_phd_data['y'], label='Master or PHD', marker='o', color='purple', linewidth=2,
         linestyle='dotted')
...
# 计算y轴的最小和最大值
y_min = min(highschool_data['y'].min(), college_data['y'].min(), bachelor_data['y'].min(),
            master_phd_data['y'].min()) - 0.5
y_max = max(highschool_data['y'].max(), college_data['y'].max(), bachelor_data['y'].max(),
            master_phd_data['y'].max()) + y_interval
...
plt.tight_layout(rect=(0, 0, 1, 0.9))
plt.ticklabel_format(style='plain', axis='y')
plt.xlim(min(highschool_data['x'].min(), college_data['x'].min(), bachelor_data['x'].min(),
             master_phd_data['x'].min()) - 0.5,
         max(highschool_data['x'].max(), college_data['x'].max(), bachelor_data['x'].max(),
             master_phd_data['x'].max()) + 0.25)
plt.grid()
...
income_0_5k['y'] = income_0_5k['y']/1000
income_5_10k['y'] = income_5_10k['y']/1000
income_10_15k['y'] = income_10_15k['y']/1000
income_15_20k['y'] = income_15_20k['y']/1000
income_above_20k['y'] = income_above_20k['y']/1000

# 绘制折线图
plt.plot(income_0_5k['x'], income_0_5k['y'], label='0~5k', marker='o', color='blue', linewidth=2, linestyle='dotted')
plt.plot(income_5_10k['x'], income_5_10k['y'], label='5~10k', marker='o', color='green', linewidth=2,
         linestyle='dotted')
plt.plot(income_10_15k['x'], income_10_15k['y'], label='10~15k', marker='o', color='orange', linewidth=2,
         linestyle='dotted')
plt.plot(income_15_20k['x'], income_15_20k['y'], label='15~20k', marker='o', color='red', linewidth=2,
         linestyle='dotted')
plt.plot(income_above_20k['x'], income_above_20k['y'], label='Above 20k', marker='o', color='purple', linewidth=2,
         linestyle='dotted')
...
# 计算y轴的最小和最大值
y_min = min(income_0_5k['y'].min(), income_5_10k['y'].min(), income_10_15k['y'].min(), income_15_20k['y'].min(),
            income_above_20k['y'].min()) - 0.5
y_max = max(income_0_5k['y'].max(), income_5_10k['y'].max(), income_10_15k['y'].max(), income_15_20k['y'].max(),
            income_above_20k['y'].max()) + y_interval
...
plt.tight_layout(rect=(0, 0, 1, 0.9))
plt.ticklabel_format(style='plain', axis='y')
plt.xlim(min(income_0_5k['x'].min(), income_5_10k['x'].min(), income_10_15k['x'].min(), income_15_20k['x'].min(),
             income_above_20k['x'].min()) - 0.25,
         max(income_0_5k['x'].max(), income_5_10k['x'].max(), income_10_15k['x'].max(), income_15_20k['x'].max(),
             income_above_20k['x'].max()) + 0.25)
plt.grid()
...
Householdlncome_data['y'] = Householdlncome_data['y']/1000
# 绘制折线图
plt.plot(Householdlncome_data['x'], Householdlncome_data['y'], label='Householdlncome', marker='o', color='blue',
...
plt.grid()
plt.savefig(os.path.join(output_folder_line, 'Householdlncome Change'))
plt.show()
#柱状图
# 设置图的标签、标题等
plt.xlabel('Year')
...
plt.legend()
output_folder_bar = '输出分类\柱状图'
plt.savefig(os.path.join(output_folder_bar, 'Householdlncome Change'))
plt.show()
#%%
People_data = pd.read_csv(os.path.join(folder_path, 'People.csv'))

People_data['y'] = People_data['y']/1000
# 绘制折线图
